...
public class TableTennisMenuChoiceController {
    ...
    @AuthorizeJwtHeader
    @PostMapping(path = "/table-tennis-menu", consumes = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void mention(@RequestBody @Nonnull final MessageSent messageSent) {
        LOGGER.info("Chose the table tennis menu controller", messageSent.getMessage().getText());
    }
